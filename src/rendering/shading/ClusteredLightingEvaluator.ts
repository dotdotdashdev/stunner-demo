import {
  ...
  type ClusterLightAssignment,
} from '../cluster/ClusterAssignment'
...
function luminance(color: Vec3): number {
  return color[0] * 0.2126 + color[1] * 0.7152 + color[2] * 0.0722
}

function evaluateLightContribution(light: RenderLight, depth: number): number {
...
export function evaluateClusteredLighting(
  lights: RenderLight[],
  ...
  const sampleX = Math.floor((Math.sin(timeSeconds * 0.4) * 0.5 + 0.5) * (gridBundle.grid.clustersX - 1))
  const sampleY = Math.floor((Math.cos(timeSeconds * 0.3) * 0.5 + 0.5) * (gridBundle.grid.clustersY - 1))
  ...
  const count = Math.min(
    assignment.counts[clusterIndex],
    config.clustered.maxLightsPerCluster,
  )
