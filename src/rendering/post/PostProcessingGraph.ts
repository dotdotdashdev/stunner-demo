  ...
  private registerPasses(): void {
    this.graph.addPass({
      ...
      execute: (context) => {
        ...
        const sceneDepth =
          6 + (Math.sin(this.state.input.timeSeconds * 0.3) * 0.5 + 0.5) * 14
        ...
        const highlight =
          0.4 + (Math.sin(this.state.input.timeSeconds * 0.7) * 0.5 + 0.5) * 0.6
