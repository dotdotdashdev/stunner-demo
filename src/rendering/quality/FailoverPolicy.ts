...
function clampPreset(preset: QualityPreset): QualityPreset {
  if (preset === 'custom') {
    return 'high'
  }
...
function presetIndex(preset: QualityPreset): number {
  return ORDERED_PRESETS.indexOf(clampPreset(preset))
}

function resolveFrameBudget(deviceClass: DeviceClass): number {
...
export function evaluateFailover(input: FailoverInput): FailoverDecision {
  ...
  if (!hasOverflow && input.avgFrameTimeMs < frameBudget * 0.72 && currentIndex < ORDERED_PRESETS.length - 1) {
    const nextPreset = ORDERED_PRESETS[currentIndex + 1]
    return {
      nextPreset,
