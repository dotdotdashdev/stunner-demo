...
export const createClusterGridInfo = (
  viewportWidth: number,
  viewportHeight: number,
  config: ClusteredConfig,
): ClusterGridInfo => {
...
export const getClusterIndex = (x: number, y: number, z: number, grid: ClusterGridInfo): number => {
  return x + y * grid.clustersX + z * grid.clustersX * grid.clustersY;
};
const clamp = (value: number, min: number, max: number): number => {
  return Math.min(max, Math.max(min, value));
};
export const depthToSlice = (
...
export const buildClusterGrid = (
  ...
  const grid = createClusterGridInfo(params.viewportWidth, params.viewportHeight, params.config);
  return {
    grid,
    policy,
    ...
    toSlice: (viewSpaceDepth: number) =>
      depthToSlice(viewSpaceDepth, params.nearPlane, params.farPlane, grid.clustersZ, policy),
  };
};
